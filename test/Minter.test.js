...
describe("Minter", function() {
  ...
  describe('minting', async function() {
    it('mints tokens as expected', async function() {
      ...
      const contract = await minter.deployed()
      // Request to mint 5 tokens and send 0.05 ETH (5 * 0.01).
      const result = await contract.mint(5, { value: ethers.utils.parseEther("0.05")})
      const totalSupply = await contract.totalSupply()
      // Success: 5 tokens should be minted.
      assert.equal(totalSupply, 5)
      // Failure: Can't mint 0 tokens with 0 ETH (0 * 0.01).
      ...
      await contract.mint(0, { value: ethers.utils.parseEther("0")}).should.be.rejected
      // Failure: Can't mint more than 5 tokens in one go.
      ...
      // Failure: Can't mint more than 5 tokens in one go, or more than the max supply.
